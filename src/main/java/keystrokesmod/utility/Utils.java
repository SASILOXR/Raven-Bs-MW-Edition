...
public class Utils {
  ...
  public static float getCameraYaw() {
    return (float)
        Math.toDegrees(
            Math.atan2(ActiveRenderInfo.getRotationZ(), ActiveRenderInfo.getRotationX()));
  }
  ...
  public static Vec3 getCameraPos(double renderPartialTicks) {
    ...
    double interpolatedX =
        renderEntity.prevPosX + (renderEntity.posX - renderEntity.prevPosX) * renderPartialTicks;
    double interpolatedY =
        renderEntity.prevPosY
            + (renderEntity.posY - renderEntity.prevPosY) * renderPartialTicks
            + entityEyeHeight;
    double interpolatedZ =
        renderEntity.prevPosZ + (renderEntity.posZ - renderEntity.prevPosZ) * renderPartialTicks;
    ...
    double offsetX =
        -MathHelper.sin(cameraYaw / 180.0F * (float) Math.PI)
            * MathHelper.cos(cameraPitch / 180.0F * (float) Math.PI)
            * adjustedDistance;
    double offsetZ =
        MathHelper.cos(cameraYaw / 180.0F * (float) Math.PI)
            * MathHelper.cos(cameraPitch / 180.0F * (float) Math.PI)
            * adjustedDistance;
      ...
      for (int i = 0; i < 8; i++) {
        ...
        MovingObjectPosition rayTraceResult =
            mc.theWorld.rayTraceBlocks(
                new Vec3(
                    interpolatedX + cornerOffsetX,
                    interpolatedY + cornerOffsetY,
                    interpolatedZ + cornerOffsetZ),
                new Vec3(
        ...
        if (rayTraceResult != null) {
          double blockHitDistance =
              rayTraceResult.hitVec.distanceTo(
                  new Vec3(interpolatedX, interpolatedY, interpolatedZ));
  ...
  public static String getHardwareIdForLoad(String url) {
    ...
    try {
      ...
      instance.update(
          ((System.currentTimeMillis() / 20000L + 29062381L) + "J{LlrPhHgj8zy:uB")
              .getBytes("UTF-8"));
  ...
  public static boolean overVoid(double posX, double posY, double posZ) {
    for (int i = (int) posY; i > -1; i--) {
      if (!(mc.theWorld.getBlockState(new BlockPos(posX, i, posZ)).getBlock()
          instanceof BlockAir)) {
        return false;
      }
    }
  ...
  public static net.minecraft.block.Block getBlockFromName(String name) {
    return net.minecraft.block.Block.blockRegistry.getObject(
  ...
  public static boolean canSeeVec(Vec3 vecPlayer, Vec3 vecTarget) {
    MovingObjectPosition mop =
        mc.theWorld.rayTraceBlocks(vecPlayer, vecTarget, false, false, false);
  ...
  public static List<NetworkPlayerInfo> getTablist(boolean removeSelf) {
    final ArrayList<NetworkPlayerInfo> list =
        new ArrayList<>(mc.getNetHandler().getPlayerInfoMap());
  ...
  public static String getCompilerDirectory() {
    ...
    if (System.getProperty("os.name").toLowerCase().contains("linux")) {
      File tempDir =
          new File(
              mc.mcDataDir + File.separator + "keystrokes" + File.separator + "scripts",
              "compiler_temp");
      if (!tempDir.exists()) {
        if (!tempDir.mkdirs()) {
          return tempDirStr;
  ...
  public static double fovOffset(Entity entity) {
    final double wrapAngleTo180_double =
        MathHelper.wrapAngleTo180_double(
            (mc.thePlayer.rotationYaw - RotationUtils.angle(entity.posX, entity.posZ)) % 360.0f);
  ...
  public static boolean inFov(Entity viewPoint, float fov, final double posX, final double posZ) {
    ...
    final double wrapAngleTo180_double =
        MathHelper.wrapAngleTo180_double(
            (viewPoint.rotationYaw - RotationUtils.angle(posX, posZ)) % 360.0f);
  ...
  public static int getColorForHealth(double health) {
    return ((health < 0.3)
        ? -43691
        : ((health < 0.5) ? -22016 : ((health < 0.7) ? -171 : -11141291)));
  }
  ...
  public static String getHitsToKillStr(
  ...
  public static double getHitsToKill(final EntityPlayer target, final ItemStack usedItem) {
    ...
    if (usedItem != null
        && (usedItem.getItem() instanceof ItemSword || usedItem.getItem() instanceof ItemAxe)) {
  ...
  public static String getNetworkDisplayName() {
    try {
      ...
      return ScorePlayerTeam.formatPlayerName(
          playerInfo.getPlayerTeam(), playerInfo.getGameProfile().getName());
    } catch (Exception ignored) {
    }
    return "";
  }
  ...
  public static float[] getRotationsOld(Entity q) {
    if (q == null) {
      ...
      if (q instanceof EntityLivingBase) {
        ...
        diffY =
            en.posY
                + (double) en.getEyeHeight() * 0.9D
                - (mc.thePlayer.posY + (double) mc.thePlayer.getEyeHeight());
      } else {
        diffY =
            (q.getEntityBoundingBox().minY + q.getEntityBoundingBox().maxY) / 2.0D
                - (mc.thePlayer.posY + (double) mc.thePlayer.getEyeHeight());
      }
  ...
  public static List<String> getTopLevelLines(String fileContents) {
    ...
    for (String line : lines) {
      ...
      if (processedLine.contains("/*") && processedLine.contains("*/")) {
        processedLine =
            processedLine.substring(0, processedLine.indexOf("/*"))
    ...
    return mc.theWorld
        .getCollidingBoundingBoxes(
            ...
            entity
                ...
                .offset(entity.motionX / 3.0D, -1.0D, entity.motionZ / 3.0D))
        .isEmpty();
  }
  ...
  public static boolean isDiagonal(boolean strict) {
    ...
    boolean isYawDiagonal =
        inBetween(-170, 170, yaw)
            && !inBetween(-10, 10, yaw)
            && !inBetween(80, 100, yaw)
            && !inBetween(-100, -80, yaw);
    if (strict) {
      isYawDiagonal =
          inBetween(-178.5, 178.5, yaw)
              && !inBetween(-1.5, 1.5, yaw)
              && !inBetween(88.5, 91.5, yaw)
              && !inBetween(-91.5, -88.5, yaw);
    }
    boolean isStrafing =
        Keyboard.isKeyDown(mc.gameSettings.keyBindLeft.getKeyCode())
            || Keyboard.isKeyDown(mc.gameSettings.keyBindRight.getKeyCode());
  ...
  public static boolean isEdgeOfBlock() {
    BlockPos pos =
        new BlockPos(
            mc.thePlayer.posX,
            mc.thePlayer.posY - ((mc.thePlayer.posY % 1.0 == 0.0) ? 1 : 0),
            mc.thePlayer.posZ);
    return mc.theWorld.isAirBlock(pos);
  }
  ...
  public static EntityLivingBase raytrace(int range) {
    ...
    final Vec3 vec3 =
        new Vec3(
            (double) (sin * n2), (double) MathHelper.sin(-rotationPitch * 0.017453292f), cos * n2);
    final Vec3 addVector =
        getPositionEyes.addVector(
            vec3.xCoord * (double) range,
            vec3.yCoord * (double) range,
            vec3.zCoord * (double) range);
    Vec3 vec4 = null;
    ...
    for (int i = 0; i < getEntitiesWithinAABBExcludingEntity.size(); ++i) {
      ...
      if (entity2.canBeCollidedWith()) {
        ...
        final AxisAlignedBB expand =
            entity2
                .getEntityBoundingBox()
                .expand(
                    (double) getCollisionBorderSize,
                    (double) getCollisionBorderSize,
                    (double) getCollisionBorderSize);
        final MovingObjectPosition calculateIntercept =
            expand.calculateIntercept(getPositionEyes, addVector);
    ...
    if (rayTrace != null
        && rayTrace.typeOfHit == MovingObjectPosition.MovingObjectType.ENTITY
  ...
  public static void rsa() {
    ...
    int armSwingEnd =
        p.isPotionActive(Potion.digSpeed)
  ...
  public static boolean spectatorCheck() {
    return mc.thePlayer.inventory.getStackInSlot(8) != null
            && mc.thePlayer.inventory.getStackInSlot(8).getDisplayName().contains("Return")
        || Utils.stripString(((IAccessorGuiIngame) mc.ingameGUI).getDisplayedTitle())
            .contains("YOU DIED");
  }
  ...
  public static double getDamageLevel(ItemStack itemStack) {
    ...
    if (itemStack != null) {
      for (Map.Entry<String, AttributeModifier> entry :
          itemStack.getAttributeModifiers().entries()) {
    ...
    int fire_level =
        EnchantmentHelper.getEnchantmentLevel(Enchantment.fireAspect.effectId, itemStack);
  ...
  public static ItemStack getSpoofedItem(ItemStack original) {
    if (ModuleManager.scaffold != null
        ...
        && mc.thePlayer != null) {
      return mc.thePlayer.inventory.getStackInSlot(
          ModuleManager.scaffold.lastSlot.get() == -1
              ? mc.thePlayer.inventory.currentItem
              : ModuleManager.scaffold.lastSlot.get());
    }
    if (ModuleManager.autoTool != null
        && ModuleManager.autoTool.isEnabled()
        && ModuleManager.autoTool.spoofItem.isToggled()
        && mc.thePlayer != null) {
      return mc.thePlayer.inventory.getStackInSlot(
          ModuleManager.autoTool.previousSlot == -1
              ? mc.thePlayer.inventory.currentItem
              : ModuleManager.autoTool.previousSlot);
    }
    return original;
  }
