...
public class KillAura extends Module {
  ...
  public KillAura() {
    ...
    this.registerSetting(
        switchDelay = new SliderSetting("Switch delay", "ms", 200.0, 50.0, 1000.0, 25.0));
    ...
    this.registerSetting(
        manualBlock = new ButtonSetting("Manual block", false)); // does absolutely nothing
  ...
  public String getInfo() {
    ...
    if (rotationMode.getInput() == 1) {
      return rotationModes[1];
    }
    switch ((int) rotationMode.getInput()) {
      case 2:
        return String.valueOf((int) this.fov.getInput());
      case 1:
        return rotationModes[1] + " " + String.valueOf((int) this.fov.getInput());
      case 0:
        if (targets.getInput() > 1) {
          return "Switch";
        } else {
          return "Single";
        }
  ...
  public void onPreUpdate(PreUpdateEvent e) {
    ...
    if (!autoBlockOverride()
        ...
        || (!manualBlock()
            && manualBlock
                .isToggled())) { // regular swing & attack if autoblock isnt overriding or isnt in
      // autoblock range
    ...
    } else if ((blinkAutoBlock() && !Utils.holdingSword())
        || !inBlockRange
        || !manualBlock()) { // for blink autoblocks
      if (blinking.get() || lag) {
        resetBlinkState(true);
        blinking.set(false);
      }
      ...
      if (inRange(target, attackRange.getInput() - 0.005)) {
        float[] rotations =
            RotationUtils.getRotations(
                target, RotationUtils.serverRotations[0], RotationUtils.serverRotations[1]);
  ...
  public void onUpdate() {
    if (rotationMode.getInput() == 1 && target != null) {
      if (inRange(target, attackRange.getInput() - 0.005)) {
        float[] rotations =
            RotationUtils.getRotations(
                target, mc.thePlayer.rotationYaw, mc.thePlayer.rotationPitch);
        float[] smoothedRotations = getRotationsSmoothed(rotations);
        mc.thePlayer.rotationYaw = smoothedRotations[0];
        mc.thePlayer.rotationPitch = smoothedRotations[1];
      }
  ...
  public void onRenderTick(TickEvent.RenderTickEvent event) {
    ...
    if (event.phase == TickEvent.Phase.START) {
      if (System.currentTimeMillis() - this.lastTime >= delay && target != null) {
        ...
        && mc.thePlayer.ticksExisted == fistTick
        && rotationMode.getInput() == 0) {
      mc.thePlayer.prevRenderArmYaw =
          mc.thePlayer.rotationYaw
              - ((mc.thePlayer.renderArmYaw - mc.thePlayer.prevRenderArmYaw) * 2.0f);
  ...
  public void onSendPacket(SendPacketEvent e) {
    ...
    if (packet instanceof C08PacketPlayerBlockPlacement) {
      ...
      if (delayTicks >= 0) {
        if (p.getStack() != null
            && p.getStack().getItem() instanceof ItemSword
            && p.getPlacedBlockDirection() != 255) {
          e.setCanceled(true);
        }
      }
    }
  ...
  public void onScrollSlot(PreSlotScrollEvent e) {
    ...
    if (stack != null
        ...
        && wasUsing
        && Utils.lookingAtBlock()) {
      onSwapSlot();
      if (Raven.debug) {
        Utils.sendModuleMessage(
            this,
            "&7Scroll swap detected, setting delay to &b"
                + delayTicks
                + "&7. (&d"
  ...
  public void onSlotUpdate(SlotUpdateEvent e) {
    ...
    if (stack != null
      ...
      if (Raven.debug) {
        Utils.sendModuleMessage(
            this,
            "&7Swap detected, setting delay to &b"
                + delayTicks
                + "&7. (&d"
                + mc.thePlayer.ticksExisted
                + "&7)");
  ...
  public void onSetAttackTarget(LivingSetAttackTargetEvent e) {
    if (e.entity != null && !hostileMobs.contains(e.entity)) {
      if (!(e.target instanceof EntityPlayer)
          || !e.target.getName().equals(mc.thePlayer.getName())) {
        return;
      }
  ...
  private void handleTarget() {
    ...
    for (Entity entity : mc.theWorld.loadedEntityList) {
      ...
      if (entity instanceof EntityPlayer) {
        ...
        if (((EntityCreature) entity).tasks == null
            || ((EntityCreature) entity).isAIDisabled()
            || ((EntityCreature) entity).deathTime != 0) { // no ai
          continue;
        }
      ...
      if (mc.thePlayer.getDistanceToEntity(entity)
          < maxRange + maxRange / 3) { // simple distance check
    ...
    for (EntityLivingBase target : availableTargets) {
      ...
      if (!(target instanceof EntityPlayer)
          ...
          && !isHostile((EntityCreature) target)) {
        continue;
      }
      if (!hitThroughBlocks.isToggled()
          && (!Utils.canPlayerBeSeen(target) || !inRange(target, attackRange.getInput() - 0.005))) {
        continue;
      }
      toClassTargets.add(
          new KillAuraTarget(
    ...
    if (!attackTargets.isEmpty()) {
      ...
      long noHitTicks =
          (long) Math.min(attackTargets.size(), targets.getInput()) * switchDelayTicks;
  ...
  private void handleSwingAndAttack(double distance, boolean swung) {
    ...
    if ((distance <= swingRange.getInput() || inAttackDistance)
        && shouldAttack
        && !swung) { // swing if in swing range or needs to attack
      if (!mc.thePlayer.isBlocking() || !disableWhileBlocking.isToggled()) {
        swingItem();
      }
    }
  ...
  private boolean isHostile(EntityCreature entityCreature) {
    if (SkyWars.onlyAuraHostiles()) {
      ...
      String teamColorSelf =
          Utils.getFirstColorCode(mc.thePlayer.getDisplayName().getFormattedText());
      if (!teamColor.isEmpty()
          && (teamColorSelf.equals(teamColor) || Utils.isTeamMate(entityCreature))) { // same team
        return false;
      }
      return true;
      ...
      if (!golems.containsKey(entityCreature.getEntityId())) {
        ...
        for (Entity entity : mc.theWorld.loadedEntityList) {
          ...
          if (stripped.contains("[") && stripped.endsWith("]")) {
            double distanceSq =
                entity.getDistanceSq(entityCreature.posX, entityCreature.posY, entityCreature.posZ);
        ...
        if (nearestArmorStand != null) {
          String teamColor =
              Utils.getFirstColorCode(nearestArmorStand.getDisplayName().getFormattedText());
          String teamColorSelf =
              Utils.getFirstColorCode(mc.thePlayer.getDisplayName().getFormattedText());
          boolean isTeam = false;
          if (!teamColor.isEmpty()
              && (teamColorSelf.equals(teamColor)
                  || Utils.isTeamMate(nearestArmorStand))) { // same team
            isTeam = true;
          }
  ...
  private void handleBlocking(boolean blockState) {
    ...
    if (autoBlockMode.getInput() != previousAutoBlockMode) {
      if (previousAutoBlockMode == 4
          || previousAutoBlockMode == 5
          || previousAutoBlockMode == 6) { // if == interact
        resetBlinkState(true);
        blinking.set(false);
      }
    }
  ...
  private double getMaxRange() {
    return Math.max(
        Math.max(swingRange.getInput(), attackRange.getInput() - 0.005), blockRange.getInput());
  }
  ...
  public boolean blinkAutoBlock() {
    return (autoBlockMode.getInput() == 4
        || autoBlockMode.getInput() == 5
        || autoBlockMode.getInput() == 6
        || autoBlockMode.getInput() == 7);
  }
  ...
  private boolean isLookingAtEntity() { //
    if (rotationMode.getInput() == 0 && rotationSmoothing.getInput() > 0) { // silent
      return RotationUtils.isPossibleToHit(
          attackingEntity, attackRange.getInput() - 0.005, RotationUtils.serverRotations);
    }
    return true;
  }
  ...
  private void handleAutoBlock(double distance) {
    ...
    boolean swung = false;
    if ((distance <= swingRange.getInput() || inAttackDistance)
        && shouldAttack) { // swing if in swing range or needs to attack
      swingItem();
      swung = true;
      if (!inAttackDistance) {
        shouldAttack = false;
  ...
  private boolean settingCondition() {
    if (requireMouseDown.isToggled() && !Mouse.isButtonDown(0)) {
      ...
      return false;
    } else if (ModuleManager.bedAura != null
  ...
  private int getBestSwapSlot() {
    ...
    if (bestSlot == -1) {
      for (int i = 0; i < 9; ++i) {
        ...
        ItemStack stack = mc.thePlayer.inventory.getStackInSlot(i);
        if (stack == null
            || Arrays.stream(swapBlacklist)
                .noneMatch(stack.getUnlocalizedName().toLowerCase()::contains)) {
          bestSlot = i;
          break;
        }
      }
  ...
  public void resetYaw() {
    ...
    float unwrappedYaw =
        unwrapYaw(MathHelper.wrapAngleTo180_float(mc.thePlayer.rotationYaw), serverYaw);
    mc.thePlayer.rotationYaw = unwrappedYaw;
    mc.thePlayer.prevRotationYaw = unwrappedYaw;
  }

  private void interactAt(
      boolean interactAt, boolean interact, boolean noEvent, boolean requireInteractAt) {
    ...
    if (interactAt) {
      ...
      MovingObjectPosition mov =
          RotationUtils.rayTrace(
              ...
              hitThroughBlocks.isToggled() ? attackingEntity : null);
      if (mov != null
          && mov.typeOfHit == MovingObjectPosition.MovingObjectType.ENTITY
          && mov.entityHit == attackingEntity) {
    ...
    if (interact) {
      if (!noEvent) {
        mc.thePlayer.sendQueue.addToSendQueue(
            new C02PacketUseEntity(attackingEntity, C02PacketUseEntity.Action.INTERACT));
      } else {
  ...
  private void handleInteractAndAttack(
      double distance, boolean interactAt, boolean interact, boolean swung) {
      ...
      if (interact) {
        mc.thePlayer.sendQueue.addToSendQueue(
            new C02PacketUseEntity(
                ModuleManager.antiFireball.fireball, C02PacketUseEntity.Action.INTERACT));
      }
    } else {
