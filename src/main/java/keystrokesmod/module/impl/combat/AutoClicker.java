...
public class AutoClicker extends Module {
  ...
  public AutoClicker() {
    ...
    this.registerSetting(
        blockHitChance = new SliderSetting("Block hit chance", "%", 0.0, 0.0, 100.0, 1.0));
  ...
  public String getInfo() {
    return String.valueOf(minCPS.getInput()) + " - " + String.valueOf(maxCPS.getInput());
  }
      ...
      if (pos != null) {
        ...
        if (block != Blocks.air && !(block instanceof BlockLiquid)) {
          if (!this.isHoldingBlockBreak
              && (!ModuleManager.killAura.isEnabled() || KillAura.target == null)) {
    ...
    if (this.nextPressTime > 0L && this.nextReleaseTime > 0L) {
      ...
      if (currentTime > this.nextPressTime
        ...
        if (mouse == 0
            ...
            && Math.random() >= (100.0 - blockHitC) / 100.0) {
