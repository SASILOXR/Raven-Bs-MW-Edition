...
public class MegaWallsItemESP extends Module {
    private ButtonSetting phxLoot,squLoot,pirLoot,renLoot,cowLoot,molLoot,molLoot1,hunLoot,diamond,diamondSword,diamondArmor;
        ...
        for (Entity entity : mc.theWorld.loadedEntityList) {
            if (entity instanceof EntityItem) {
                ...
                if (stripItemName.startsWith("Phoenix's Tears of Regen") && phxLoot.isToggled()) {
                        this.renderItemTag(entity, "§6Phoenix's Tears of Regen §fx" + stackSize, -1, event.partialTicks, itemTagScale.getInput());
                    }

                    if (stripItemName.startsWith("Squid's Absorption") && squLoot.isToggled()) {
                        this.renderItemTag(entity, "§9Squid's Absorption §fx" + stackSize, -1, event.partialTicks, itemTagScale.getInput());
                    }

                    if (stripItemName.startsWith("Matey") && pirLoot.isToggled()) {
                        this.renderItemTag(entity, itemName + " §fx" + stackSize, -1, event.partialTicks, itemTagScale.getInput());
                    }

                    if (stripItemName.startsWith("Regen-Ade") && renLoot.isToggled()) {
                        this.renderItemTag(entity, "§bRegen-ades §fx" + stackSize, -1, event.partialTicks, itemTagScale.getInput()); }

                    if (stripItemName.startsWith("Ultra Pasteurized Milk Bucket") && cowLoot.isToggled()) {
                        this.renderItemTag(entity, "§fMilk Bucket §fx" + stackSize, -1, event.partialTicks, itemTagScale.getInput());
                    }
                    ...
                    if (item == Items.pumpkin_pie && molLoot1.isToggled()) {
                        this.renderItemTag(entity, itemName + " §fx" + stackSize, 16711610, event.partialTicks, itemTagScale.getInput());
                    }

                    if (item == Items.golden_apple && hunLoot.isToggled()) {
                        this.renderItemTag(entity, itemName + " §fx" + stackSize, 16776260, event.partialTicks, itemTagScale.getInput());
                    }

                    if (item == Items.diamond && diamond.isToggled()) {
                        this.renderItemTag(entity, itemName + " §fx" + stackSize, 3132391, event.partialTicks, itemTagScale.getInput());
                    }
    ...
    private void renderItemTag(Entity entity, String text, int color, float partialTicks, double scale) {
