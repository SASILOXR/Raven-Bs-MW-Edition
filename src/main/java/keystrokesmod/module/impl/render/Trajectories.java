...
public class Trajectories extends Module {
  ...
  public void onRenderWorldLast(RenderWorldLastEvent e) {
    ...
    boolean usingBow = heldItem instanceof ItemBow;
    if (!usingBow
        && !(heldItem instanceof ItemSnowball)
        && !(heldItem instanceof ItemEgg)
        && !(heldItem instanceof ItemEnderPearl)) {
      return;
    }
    if (usingBow && !mc.thePlayer.isUsingItem() && disableUnchargedBow.isToggled()) {
      return;
    }
    ...
    double arrowMotion =
        Math.sqrt(
            arrowMotionX * arrowMotionX
                + arrowMotionY * arrowMotionY
    ...
    for (int i = 0; i < 750; i++) {
      posList.add(
          new double[] {
            arrowPosX - renderManager.viewerPosX,
            arrowPosY - renderManager.viewerPosY,
            arrowPosZ - renderManager.viewerPosZ
          });
      Vec3 arrowVec = new Vec3(arrowPosX, arrowPosY, arrowPosZ);
      Vec3 arrowVecNext =
          new Vec3(arrowPosX + arrowMotionX, arrowPosY + arrowMotionY, arrowPosZ + arrowMotionZ);
      double size = 0.5;
      AxisAlignedBB arrowBox =
          new AxisAlignedBB(
                  arrowPosX - size,
                  arrowPosY - size,
                  arrowPosZ - size,
              ...
              .expand(1, 1, 1);
      List<Entity> list =
          mc.theWorld.getEntitiesWithinAABBExcludingEntity(mc.getRenderViewEntity(), arrowBox);
      double minDistSq = 2.0;
      for (final Entity en : list) {
        if (en instanceof EntityLivingBase
          ...
          AxisAlignedBB axis =
              en.getEntityBoundingBox()
                  .expand(0.30000001192092896, 0.30000001192092896, 0.30000001192092896);
    ...
    if (entity != null && block != null) {
      if (mc.thePlayer.getDistanceToEntity(entity)
          >= mc.thePlayer.getDistanceSqToCenter(block.getBlockPos())) {
        entity = null;
        facingEntity = null;
      } else {
        block = null;
    ...
    if (facing != null) {
      double size2 = autoScale.isToggled() ? Math.min(0.1 * (1 + distSq2 / 500), 0.5) : 0.1;
      switch (facing) {
        case WEST:
        case EAST:
          {
