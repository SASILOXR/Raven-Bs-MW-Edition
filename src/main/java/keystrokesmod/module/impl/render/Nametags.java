...
public class Nametags extends Module {
  ...
  private ButtonSetting deepBlackBackground;
  ...
  private int deepBackGroundColor = new Color(0, 0, 0, 230).getRGB();
  ...
  public Nametags() {
    ...
    this.registerSetting(deepBlackBackground = new ButtonSetting("Deep black Background", false));
  ...
  public void onRenderWorldLast(RenderWorldLastEvent ev) {
    ...
    if (mc.gameSettings.thirdPersonView > 0) {
      ...
      interpolatedX =
          mc.thePlayer.lastTickPosX
              + (mc.thePlayer.posX - mc.thePlayer.lastTickPosX) * ev.partialTicks;
      interpolatedY =
      ...
      interpolatedZ =
          mc.thePlayer.lastTickPosZ
              + (mc.thePlayer.posZ - mc.thePlayer.lastTickPosZ) * ev.partialTicks;
    }
    ...
    for (EntityPlayer en : mc.theWorld.playerEntities) {
      ...
      ((IAccessorEntityRenderer) mc.entityRenderer)
          .callSetupCameraTransform(((IAccessorMinecraft) mc).getTimer().renderPartialTicks, 0);

      Vec3 screenCords =
          RenderUtils.convertTo2D(
          ...
          if (deltaZ < normalizedThreshold + 1) {
            ...
            if (deltaY < normalizedThreshold + 1) {
              ...
              if (distance < normalizedThreshold) {
                newScale =
                    Math.max(
                        (float) (scaleSetting * (normalizedThreshold / distance)), scaleSetting);
              }
            }
          }
        }
      ...
      if (drawBackground.isToggled()) {
        if (deepBlackBackground.isToggled()) {

          RenderUtils.drawRect(x1, y1, x2, y2, deepBackGroundColor);
        } else {
  ...
  public void onRenderLiving(RenderLivingEvent.Specials.Pre e) {
    if (e.entity instanceof EntityPlayer
        && (e.entity != mc.thePlayer || renderSelf.isToggled())
        && e.entity.deathTime == 0) {
      EntityPlayer entityPlayer = (EntityPlayer) e.entity;
      if (!showInvis.isToggled() && entityPlayer.isInvisible()) {
        return;
      }
      if (entityPlayer.getDisplayNameString().isEmpty()
          || (entityPlayer != mc.thePlayer && AntiBot.isBot(entityPlayer))) {
        return;
      }
      e.setCanceled(true);
    }
  ...
  private void renderText(ItemStack stack, int xPos, int yPos) {
    ...
    if (showDurability.isToggled() && stack.getItem() instanceof ItemArmor) {
      ...
      mc.fontRendererObj.drawString(
          String.valueOf(remainingDurability),
          (float) (xPos * 2),
          (float) yPos,
          16777215,
      ...
      if (stack.getItem() instanceof ItemTool
        ...
        for (int i = 0; i < nbttaglist.tagCount(); ++i) {
          ...
          if (lvl > 0) {
            ...
            mc.fontRendererObj.drawString(
                abbreviated + lvl, (float) (xPos * 2), (float) newYPos, -1, dropShadow.isToggled());
            newYPos += 8;
          }
        }
      }
    }
    if (showStackSize.isToggled()
        && !(stack.getItem() instanceof ItemSword)
        && !(stack.getItem() instanceof ItemBow)
        && !(stack.getItem() instanceof ItemTool)
        && !(stack.getItem() instanceof ItemArmor)) {
      mc.fontRendererObj.drawString(
          stack.stackSize + "x", (float) (xPos * 2), (float) yPos, -1, dropShadow.isToggled());
    }
  }
